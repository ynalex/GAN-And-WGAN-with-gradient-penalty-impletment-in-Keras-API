...
from loss.loss import generator_loss, discriminator_loss, gradient_penalty
...
@tf.function
def train_discriminator(generator, discriminator, discriminator_optimizer, real_image, batch_size=64, z_dim=128, gradient_penalty_weight=10):
    with tf.GradientTape() as tape:
        ...
        gp = gradient_penalty(partial(discriminator, training=True),real_image, fake_image)
...
def train_gan(
    ...
    train_data_list,
    train_ratio=6,
    batch_size=64, z_dim=128, gradient_penalty_weight=10, epochs=30):
    ...
    for iters in range(epochs):
        ...
        for step, real_image in enumerate(train_data_list):
            ...
            d_loss = train_discriminator(generator, discriminator, discriminator_optimizer, real_image, batch_size, z_dim, gradient_penalty_weight)
